#!/usr/bin/env node
/**
 * Seed the Shokudo org chart.
 *
 * Run from whg-team-portal directory:
 *   node scripts/seed-org-chart-shokudo.mjs
 *
 * Idempotent — clears existing Shokudo org chart rows before inserting.
 */
...
async function main() {
  ...
  if (!restaurants || restaurants.length === 0) {
    console.error('Could not find Shokudo in restaurants table');
    process.exit(1);
  }
  const restId = restaurants[0].id;
  console.log(`Found Shokudo: ${restId}`);
  ...
  const { error: csErr } = await supabase
    ...
    .insert([{
      ...
      role_level: 2,
      sort_order: 1,
      detail: 'Management and staff positions will be added as Shokudo launches',
      reports_to: randyId,
      photo_url: null,
      active: true,
    }]);
  ...
  console.log(`\nDone. Seeded ${insertedOwners.length + 1} org chart positions for Shokudo.`);
}
